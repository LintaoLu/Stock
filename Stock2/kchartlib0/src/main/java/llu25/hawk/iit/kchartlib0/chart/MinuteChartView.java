...
import llu25.hawk.iit.kchartlib0.chart.entity.IMinuteLine;
...
public class MinuteChartView extends View implements GestureDetector.OnGestureListener {
    ...
    private void init() {
        ...
        mVolumePaintGreen.setColor(ContextCompat.getColor(getContext(), R.color.chart_red));
        mVolumePaintRed.setColor(ContextCompat.getColor(getContext(),R.color.chart_green));
    ...
    private void drawValue(Canvas canvas, int index) {
        ...
        if (index >= 0 && index < mPoints.size()) {
            ...
            String text = "STRIKE PRICE:" + floatToString(point.getPrice()) + " ";
            float x = 0;
            canvas.drawText(text, x, y, mPricePaint);
            x += mPricePaint.measureText(text);
            text = "AVG PRICE:" + floatToString(point.getAvgPrice()) + " ";
